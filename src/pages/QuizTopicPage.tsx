...
import { QuizTopic, QuizTopicAIInsights } from '../types/types';

const QuizTopicPage = () => {
    const { topicId } = useParams<{ topicId: string }>();
    const navigate = useNavigate();

    const { fetchQuizTopic, fetchQuizTopicAIInsights } = useAttemptStore();

    const [topic, setTopic] = useState<QuizTopic | null>(null);
    const [topicLoading, setTopicLoading] = useState(true);
    const [topicError, setTopicError] = useState(false);

    const [insights, setInsights] = useState<QuizTopicAIInsights | null>(null);
    ...
    useEffect(() => {
        ...
        fetchQuizTopic(Number(topicId)).then((data) => {
            if (data) {
                setTopic(data);
            } else {
                setTopicError(true);
    ...
    }, [topicId, fetchQuizTopic]);

    const handleGetInsights = async () => {
        if (!topicId) return;
        setInsightsLoading(true);
        setInsightsError('');
        const data = await fetchQuizTopicAIInsights(Number(topicId));
        if (data) {
            setInsights(data);
        } else {
                        ...
                        <div>
                            ...
                            {topic.url && (
                                <a
                                    ...
                                    className="flex items-center gap-1 text-indigo-600 hover:underline plain-text"
                                >
